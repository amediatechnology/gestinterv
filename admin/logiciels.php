...
if ( !empty($_POST) && (($_POST["verif"])=="ajout-logiciel") ) // Si les variables "$_POST" sont vides,
{
	$logiciel = addslashes($_POST["logiciel"]); // Récupération nom logiciel + Sécurité caractères spéciaux
...
<?php 
	...
	{ // On affiche les lignes du tableau
		...
		echo "<td align='center'><form action='admin/dellogiciel.php?id=" . $ligne["id"] . "' method='POST'> <input type='hidden' name='id' value='".$ligne["id"]."'> <input type='submit' value='Suppression' onclick=\"javascript:return(confirm('Confirmer la suppression ?'))\";> </form></td>" ; // Bouton de suppression du logiciel (fait appel à la page dellogiciel.php)
